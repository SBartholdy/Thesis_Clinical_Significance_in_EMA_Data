...
#save(EMA_Class, file = "cor_04_k20/EMA_Class_n50.RData")
...
EMA_Class %<>%
  dplyr::mutate(Mean_PC_5.5_Days = recode(Mean_PC_5.5_Days, '-2' = 1L, '-1' = 0L, '0' = 0L, '1' = 0L, '2' = -1L)) %>% 
  dplyr::mutate(across(.cols = everything(), as.factor(exclude = ...)))
...
for (i in 1:ncol(EMA_Class)) {
  ...
  #EMA_Class[which(x %notin% levels(EMA_Class$CSI_30.30)),i] = NA
  
  #droplevels(x, exclude = c("1"))
  #droplevels(x, exclude = which(levels(x) %notin% levels(EMA_Class$CSI_30.30)))
}

droplevels(EMA_Class$CSI_5.5_Days, exclude = "1")
#levels(EMA_Class$RCI_ind_preSD_30.30) = c("-1", "0", "1")
...
for (i in 1:ncol(EMA_Class)) {
  ...
  ClassEval[[paste0(names(EMA_Class[,i]))]] = cm[c("table","byClass","agreement")]
}

#save(ClassEval, file = "cor_04_k20/EMA_ClassEval_n50.RData")
#load("cor_04_k20/EMA_Class_n50.RData")
...
#save(SenSpezSumm, file = "cor_04_k20/EMA_SenSpezSumm_n50.RData")
#load("cor_04_k20/EMA_SenSpezSumm_n50.RData")
